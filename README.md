...
| **Forge** | 70-95% bash output | That `npm install` with 400 lines? Compressed to 15. Re-running the same build? Now you only see the 2-line diff. |
| **Agents** | 35-55% context tokens | 19 specialists with focused prompts beat one confused generalist every time |
...
| **Squire** | `@runner` | Quick verifications, simple checks, build/test confirmation (ultra-cheap) |
...
- Differential Output (Diff Cache): Re-runs of the same command only show the changes
- Content Summaries (Read Cache): Re-reading the same file? You get a 1-line summary instead of 500 lines of code
- Error Compression: Groups repetitive TS/ESLint errors into compact summaries
- Deduplication and redundancy cache (Jaccard similarity + FNV1a hashing)
...
All 19 agents receive the Vault protocol via `system.transform` injection. They know to `mem_search` before acting and `mem_save` their key findings when done.
...
- System Prompt Injection: Codex injects a compact file map directly into the agent's brain so they know what exists without asking
